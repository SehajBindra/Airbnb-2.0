...
function Header() {
  return (
    <header className="sticky z-50 shadow-md top-0    sm:grid sm:grid-cols-3 bg-white p-5 md:px-10">
      ...
      {/* center */}
      <div className=" flex items-center border-2 rounded-full p-3 pl-4">
        <input
          className=" focus-within:outline-none bg-transparent flex items-center flex-grow"
          type="text"
          placeholder="Search Your dates"
        />
        <MagnifyingGlassIcon className="absolute right-8 sm:inline-flex sm:relative sm:mr-6 h-8 text-white bg-red-400 rounded-full p-2 cursor-pointer" />
      </div>
